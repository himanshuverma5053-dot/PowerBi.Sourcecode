  ...
  return (
    <div className="space-y-8 py-6">
      ...
      <div className="bg-gradient-to-r from-purple-950 via-purple-900 to-indigo-950 text-white p-6 sm:p-8 rounded-3xl border border-purple-800/60 shadow-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-amber-400/15 border border-amber-400/30 text-amber-300 text-xs font-semibold mb-2">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white border border-purple-100 shadow-sm rounded-2xl p-4 flex flex-col md:flex-row gap-4 justify-between items-center">
          <div className="relative w-full md:w-96">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-purple-600" />
            <input
              type="text"
              placeholder="Search by order #, name, or GSTIN..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2 bg-purple-50/50 border border-purple-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-purple-600 focus:bg-white transition-all"
            />
          </div>
            ...
            {['All', 'Confirmed', 'Dispatched', 'Out for Delivery', 'Delivered'].map((status) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-all ${
                  statusFilter === status
                    ? 'bg-purple-900 text-white border border-purple-900 shadow-sm'
                    : 'bg-purple-50/70 text-slate-700 border border-purple-200 hover:bg-purple-100'
                }`}
              >
                {status}
              </button>
        ...
        {filteredOrders.length === 0 ? (
          <div className="bg-white border border-purple-100 rounded-3xl p-12 text-center space-y-4 shadow-sm">
            <Package className="w-12 h-12 text-purple-300 mx-auto" />
            ...
            <button
              ...
              className="mt-2 inline-flex items-center space-x-2 px-5 py-2.5 bg-purple-900 hover:bg-purple-950 text-white font-bold rounded-xl shadow-md transition-colors"
            >
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredOrders.map((order, idx) => (
              <div
                ...
                className="bg-white border border-purple-100 hover:border-purple-300 transition-all rounded-2xl p-5 sm:p-6 space-y-4 shadow-md"
              >
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-purple-100 pb-4">
                  <div>
                    ...
                    <div className="flex items-center space-x-4 text-xs text-slate-600 mt-1">
                      ...
                      {order.gstNumber && (
                        <span className="text-purple-900 bg-purple-50 px-2 py-0.5 rounded border border-purple-200 font-mono font-bold">
                  ...
                  <div className="flex items-center space-x-2 w-full sm:w-auto justify-end">
                    <button
                      ...
                      className="px-3.5 py-2 rounded-xl bg-purple-100 hover:bg-purple-200 border border-purple-200 text-xs font-bold text-purple-950 flex items-center space-x-1.5 transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="px-3.5 py-2 rounded-xl bg-purple-900 hover:bg-purple-950 text-white text-xs font-extrabold flex items-center space-x-1.5 transition-colors shadow-sm"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {order.items.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex items-center space-x-3 p-2.5 bg-purple-50/50 rounded-xl border border-purple-100"
                    >
                      <img
                        src={item.product.image}
                        alt={item.product.name}
                        className="w-12 h-12 object-cover rounded-lg bg-white border border-purple-100"
                ...
                {/* Footer Totals */}
                <div className="flex flex-wrap items-center justify-between gap-4 pt-2 text-xs border-t border-purple-100 text-slate-600">
                  <div className="flex items-center space-x-4">
                    ...
                    {order.trackingNumber && (
                      <span>
                        Tracking #: <strong className="text-purple-900 font-mono font-bold">{order.trackingNumber}</strong>
                      </span>
                    )}
                  </div>
