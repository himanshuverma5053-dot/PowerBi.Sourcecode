...
export const ContinuousProductBar: React.FC<ContinuousProductBarProps> = ({
  ...
  return (
    <div className="my-1.5 space-y-0.5 bg-white rounded-xl p-2 sm:p-2.5 shadow-2xs overflow-hidden relative group/bar">
      {/* CONTINUOUS MOVING CAROUSEL MARQUEE TRACK */}
      <div className="relative overflow-hidden py-0.5 md:py-1 -mx-2 sm:-mx-2.5 px-2 sm:px-2.5">
        {/* Visual edge gradient overlays */}
        <div className="absolute left-0 top-0 bottom-0 w-6 sm:w-8 md:w-10 bg-gradient-to-r from-white via-white/80 to-transparent z-10 pointer-events-none" />
        <div className="absolute right-0 top-0 bottom-0 w-6 sm:w-8 md:w-10 bg-gradient-to-l from-white via-white/80 to-transparent z-10 pointer-events-none" />
        ...
        >
          {displayItems.map((product, idx) => {
            ...
            return (
              <div
                key={`${product.id}-${idx}`}
                className="w-36 sm:w-40 md:w-44 shrink-0 mx-1 group/card bg-slate-50 hover:bg-white rounded-xl p-2 transition-all duration-200 hover:shadow-xs flex flex-col justify-between select-none relative"
              >
                {/* Product Image & Badges */}
                <div
                  className="relative w-full h-16 sm:h-20 md:h-24 rounded-lg bg-slate-100 overflow-hidden flex items-center justify-center p-1 mb-1 select-none"
                >
                ...
                <div className="space-y-0.5 select-none">
                  {(sizeString || product.category) && (
                    <div className="text-[9px] font-extrabold text-slate-700 uppercase tracking-wider flex items-center justify-between">
                      ...
                      {product.category && <span className="text-[8px] text-slate-500 font-semibold">{product.category}</span>}
                    </div>
                  )}
                  <h3 className="text-[11px] md:text-xs font-black text-slate-900 line-clamp-1 group-hover/card:text-slate-700 transition-colors">
                    {product.name}
                  </h3>
                ...
                {/* Action Buttons */}
                <div className="mt-1 pt-1 border-t border-slate-200 flex items-center gap-1">
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    className="flex-1 py-1 px-1.5 rounded-lg bg-[#9800ff] hover:bg-[#8500df] active:bg-[#7200be] active:scale-95 text-white font-extrabold text-[10px] shadow-2xs transition-all flex items-center justify-center space-x-1 cursor-pointer"
                  >
