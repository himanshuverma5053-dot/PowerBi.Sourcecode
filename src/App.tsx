...
export default function App() {
  ...
  // Open Order Details / Confirm Details Page on Buy Now or Add to Cart Click
  const handleInstantBuy = (product: TyreProduct, quantity: number = 1) => {
    setCheckoutOrderProduct({ product, quantity: Math.max(1, quantity) });
    ...
    <div className="min-h-screen bg-[#F7F7F7] text-slate-900 font-sans flex flex-col selection:bg-slate-900 selection:text-white overflow-x-hidden w-full max-w-full">
      ...
      {interfaceMode === 'admin' ? (
        ...
        {activeTab === 'catalogue' && (() => {
          ...
          return (
            <div id="products-container" className="max-w-[325px] sm:max-w-[345px] mx-auto px-2 sm:px-2.5 py-7 sm:py-9 space-y-10 sm:space-y-12">
              {/* Products Header Card */}
              <div className="bg-white px-3.5 py-4 sm:px-4 sm:py-5 rounded-xl border border-slate-200/90 shadow-2xs space-y-3 w-full">
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between py-1">
                    <h1 className="text-xl sm:text-2xl font-black font-display text-slate-900 tracking-tight leading-normal">
                      Products
                    </h1>
                    {isProductsLoading && (
                      <span className="inline-flex items-center gap-1.5 text-[9px] text-[#8a18ca] font-semibold bg-purple-50 px-2.5 py-1 rounded-full">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#8a18ca] animate-pulse" />
                        <span>Updating...</span>
                      </span>
                    )}
                  </div>
                  ...
                  {/* Controls: Search Bar */}
                  <div className="w-full relative flex items-center mt-1">
                    <CustomSearchIcon className="w-3.5 h-3.5 text-slate-400 absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none" />
                    <input
                      type="text"
                      placeholder="Search tyres..."
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="w-full pl-7.5 pr-6 py-1 h-8 sm:h-8.5 rounded-lg bg-white border border-slate-300 text-xs sm:text-[13px] font-semibold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-[#8a18ca] transition-all shadow-2xs"
                    />
                    {searchQuery && (
                      <button
                        onClick={() => setSearchQuery('')}
                        className="absolute right-1.5 top-1/2 -translate-y-1/2 p-0.5 text-slate-400 hover:text-slate-900 rounded-full hover:bg-slate-200 transition-colors cursor-pointer"
        ...
        {activeTab === 'order-details' && (
          <OrderDetailsPage
            product={
              (checkoutOrderProduct?.product?.id && products.find(p => p.id === checkoutOrderProduct.product.id)) ||
              checkoutOrderProduct?.product ||
              visibleProducts[0] ||
              MOCK_TYRES[0]
            }
            ...
            onNavigateToProfile={(targetField?: string) => {
              const target = targetField || 'workshop_address';
              setProfileTargetSection(target);
              try {
                sessionStorage.setItem('profile_target_focus', target);
              } catch (err) {
                // ignore
              }
        ...
        {activeTab === 'account' && (
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <ProfilePage
              ...
              customerAccounts={customerAccounts}
              onUpdateCustomerAccounts={setCustomerAccounts}
              initialFocusSection={profileTargetSection}
              onClearFocusSection={() => setProfileTargetSection(null)}
            />
          </div>
        )}
